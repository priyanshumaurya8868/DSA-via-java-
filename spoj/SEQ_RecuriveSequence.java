...
public class SEQ_RecuriveSequence {
...
//		 }
		...
		//preparing an array that'll contain a[i], a[i-1],a[i-2]...a[i-k] 
		//from another array that contains  a[i-k=0],a[i-k+1],...a[i-2],a[i-1]
 ...
 private static long[] multiply(long[][] trans,int[] a) 
 {
	...
	for(int  i=0 ;i < trans.length; i++) {
		for(int j =0 ; j <trans.length; j++) {
			res[i] += (trans[i][j]*a[a.length-1-j])%MOD;  // a(i) = c1*a(i-1) + c2*a(i-2) + ... + ck*a(i-k) (for i > k)
			res[i] %= MOD;
		}
	}
	return res;
	...
	private static long[][] buildTrans(int k, int[] b) {
		long[][] trans = new long[k][k]; 
		for(int  i=0; i < k; i++) trans[0][i] = b[i];
